...
var _ desync.WriteStore = &RemoteHTTP{}
...
func NewRemoteHTTPStoreBase(location *url.URL, opt desync.StoreOptions) (*RemoteHTTPBase, error) {
	...
	return &RemoteHTTPBase{location: location, client: client, opt: opt, converters: nil}, nil
}
...
func (r *RemoteHTTPBase) IssueHttpRequest(method string, u *url.URL, rangeHeader string, getReader GetReaderForRequestBody, attempt int) (int, []byte, error) {
...
func (r *RemoteHTTPBase) IssueRetryableHttpRequest(method string, u *url.URL, rangeHeader string, getReader GetReaderForRequestBody) (int, []byte, error) {
...
retry:
	attempt++
	statusCode, responseBody, err := r.IssueHttpRequest(method, u, rangeHeader, getReader, attempt)
...
func (r *RemoteHTTPBase) GetObject(name string, rangeHeader string) ([]byte, error) {
	u, _ := r.location.Parse(name)
	statusCode, responseBody, err := r.IssueRetryableHttpRequest("GET", u, rangeHeader, func() io.Reader { return nil })
	if err != nil {
		return nil, err
	}
	switch statusCode {
	...
	case 404:
		return nil, fmt.Errorf("404: no such object %s", name)
	default:
...
func NewRemoteHTTPStore(location *url.URL, opt desync.StoreOptions) (*RemoteHTTP, error) {
...
func (r *RemoteHTTP) GetChunk(id desync.ChunkID) (*desync.Chunk, error) {
	panic("ouch")
	// p := r.nameFromID(id)
	// b, err := r.GetObject(p)
	// if err != nil {
...
func (r *RemoteHTTP) HasChunk(id desync.ChunkID) (bool, error) {
...
func (r *RemoteHTTP) StoreChunk(chunk *desync.Chunk) error {
	panic("not implemented")
